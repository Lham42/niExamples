//==========================================================================
//
// Title:       SelfTestAllDevices.c
// Purpose:     Self-Test all hardware devices on the specified system
//																
//==========================================================================
...
#include <ansi_c.h>
#include "nisyscfg.h"
#include "SelfTestAllDevices.h"

int panelHandle = 0;

int main(void)
{
	panelHandle = LoadPanel(0, "SelfTestAllDevices.uir", PANEL);
...
int SelfTestDevices(void)
{
	...
	if (resourcesHandle)
	{
		...
		while (NISysCfg_Succeeded(status) && (status = NISysCfgNextResource(session, resourcesHandle, &resource)) == NISysCfg_OK)
		{
			...
			status = NISysCfgSelfTestHardware(resource, 0, &detailedResult);
		
			// 	Populate table with results of self-test
			switch (status)
			{
				case NISysCfg_OK:
		...
		MessagePopup("No Devices Found", "No devices were found that can be self-tested on the specified system");
